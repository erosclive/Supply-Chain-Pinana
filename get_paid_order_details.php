...
try {
    // Query to get all paid orders with their payment details
    $query = "SELECT 
                ro.*,
                r.retailer_name, 
                r.retailer_email, 
              ...
              FROM 
                retailer_orders ro
              LEFT JOIN 
                retailers r ON ro.retailer_id = r.retailer_id
              WHERE 
                ro.payment_status = 'paid' OR ro.payment_status = 'partial'
              ORDER BY 
                ro.order_date DESC";
    ...
    while ($order = $result->fetch_assoc()) {
        ...
        // Get item payment details for this order
        $itemPaymentsQuery = "SELECT 
                                roip.*,
                                rop.payment_method,
                                rop.payment_reference,
                                rop.payment_amount,
                                rop.created_at as payment_date,
                                p.product_name
                              FROM 
                                retailer_order_items_payments roip
                              JOIN 
