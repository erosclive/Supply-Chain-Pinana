...
function getFixedPineappleSupplier() {
    ...
    // Get alternatives for fixed pineapple supplier
    ...
    if ($altResult) {
        while ($altRow = mysqli_fetch_assoc($altResult)) {
            $alternative = [
                ...
                'contactInfo' => $altRow['contact_info'],
                'link' => $altRow['link'],
                'farmLocation' => $altRow['farm_location'],
...
// Function to get fixed pineapple alternatives
function getFixedPineappleAlternatives() {
    global $conn;
    
    $sql = "SELECT * FROM supplier_alternatives WHERE is_fixed_pineapple = 1";
    $result = mysqli_query($conn, $sql);
...
function addSupplier($data) {
    ...
    if ($type === 'physical') {
        ...
        $sql .= ", link, platform, delivery_info";
        $values .= ", '$link', '$platform', '$deliveryInfo'";
    }
...
function updateSupplier($data) {
    ...
    if ($type === 'physical') {
        ...
        $sql .= ", link = '$link', platform = '$platform', delivery_info = '$deliveryInfo'";
    }
...
// Function to update fixed pineapple supplier
...
// Update the addAlternativeSupplier function to include better error handling and debugging

// Function to add an alternative supplier
function addAlternativeSupplier($data) {
    global $conn;
    
    try {
        // Log that the function is being called
        error_log("addAlternativeSupplier called");
        ...
        error_log("Adding alternative supplier with data: " . json_encode($data));
        ...
        if ($isFixedPineapple) {
            ...
            // Insert without supplier_id for fixed pineapple alternatives
            ...
            // Check if a similar record already exists to prevent duplicates
            $checkDuplicateSql = "SELECT id FROM supplier_alternatives WHERE 
                                name = '$name' AND 
                                supplier_id = '$supplierId' AND 
                                is_fixed_pineapple = 0 AND 
                                created_at > DATE_SUB(NOW(), INTERVAL 1 MINUTE)";
